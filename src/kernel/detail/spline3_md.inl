...
template <
    ...
    bool WORKFLOW,
    typename INTERP>
__forceinline__ __device__ void interpolate_stage_md_1(
    volatile T1 s_data[AnchorBlockSizeZ * numAnchorBlockZ + (SPLINE_DIM >= 3)]
    [AnchorBlockSizeY * numAnchorBlockY + (SPLINE_DIM >= 2)]
    [AnchorBlockSizeX * numAnchorBlockX + (SPLINE_DIM >= 1)],
volatile T2 s_ectrl[AnchorBlockSizeZ * numAnchorBlockZ + (SPLINE_DIM >= 3)]
    ...
    int NUM_ELE)
{
    ...
    auto run = [&](auto x, auto y, auto z) {
        ...
        if (xyz_predicate<SPLINE_DIM,
           ...
           T1 tmp_z[5], tmp_y[5], tmp_x[5];
           int id_z[5], id_y[5], id_x[5];
           id_z[0] = (z - 3 * unit >= 0) ? z - 3 * unit : 0;
           id_z[1] = (z - unit >= 0) ? z - unit : 0;
           id_z[2] = (z + unit <= AnchorBlockSizeZ * numAnchorBlockZ) ? z + unit : 0;
           id_z[3] = (z + 3 * unit <= AnchorBlockSizeZ * numAnchorBlockZ) ? z + 3 * unit : 0;
           id_z[4] = (z + 5 * unit <= AnchorBlockSizeZ * numAnchorBlockZ) ? z + 5 * unit : 0;
           
           id_y[0] = (y - 3 * unit >= 0) ? y - 3 * unit : 0;
           id_y[1] = (y - unit >= 0) ? y - unit : 0;
           id_y[2] = (y + unit <= AnchorBlockSizeY * numAnchorBlockY) ? y + unit : 0;
           id_y[3] = (y + 3 * unit <= AnchorBlockSizeY * numAnchorBlockY) ? y + 3 * unit : 0;
           id_y[4] = (y + 5 * unit <= AnchorBlockSizeY * numAnchorBlockY) ? y + 5 * unit : 0;
           
           id_x[0] = (x - 3 * unit >= 0) ? x - 3 * unit : 0;
           id_x[1] = (x - unit >= 0) ? x - unit : 0;
           id_x[2] = (x + unit <= AnchorBlockSizeX * numAnchorBlockX) ? x + unit : 0;
           id_x[3] = (x + 3 * unit <= AnchorBlockSizeX * numAnchorBlockX) ? x + 3 * unit : 0;
           id_x[4] = (x + 5 * unit <= AnchorBlockSizeX * numAnchorBlockX) ? x + 5 * unit : 0;
            ...
            if CONSTEXPR (LINE) {
                ...
                int right_bound = AnchorBlockSizeX * numAnchorBlockX + (SPLINE_DIM >= 1);
                int x_size = AnchorBlockSizeX * numAnchorBlockX + (SPLINE_DIM >= 1);
                int y_size = AnchorBlockSizeY * numAnchorBlockY + (SPLINE_DIM >= 2);
                int z_size = AnchorBlockSizeZ * numAnchorBlockZ + (SPLINE_DIM >= 3);
                ...
                id_[4] = input_x + 5 * unit;
                id_[4] = id_[5] < right_bound ? id_[4] : 0;
                
                s_id[0] = x_size * y_size * z + x_size * y + id_[0];
                s_id[1] = x_size * y_size * z + x_size * y + id_[1];
                s_id[2] = x_size * y_size * z + x_size * y + id_[2];
                s_id[3] = x_size * y_size * z + x_size * y + id_[3];
                s_id[4] = x_size * y_size * z + x_size * y + id_[4];
                int stride = 2 * unit;
                ...
                s_id[2] = x_size * y_size * id_[2] + x_size * y + x;
                s_id[3] = x_size * y_size * id_[3] + x_size * y + x;
                s_id[4] = x_size * y_size * id_[4] + x_size * y + x;
                stride = 2 * unit * x_size * y_size;
                }
                else if (I_Y){
                    s_id[0] = x_size * y_size * z + x_size * id_[0] + x;
                    s_id[1] = x_size * y_size * z + x_size * id_[1] + x;
                    s_id[2] = x_size * y_size * z + x_size * id_[2] + x;
                    s_id[3] = x_size * y_size * z + x_size * id_[3] + x;
                    s_id[4] = x_size * y_size * z + x_size * id_[4] + x;
                    stride = 2 * unit * x_size;
                }
                ...
                case_line[0] = (input_BI != input_GD - 1);
                case_line[1] = (input_x >= 3 * unit);
                case_line[2] = (input_x + 3 * unit <= AnchorBlockSizeX * numAnchorBlockX);
                case_line[3] = (input_gx + 3 * unit < input_gs);
                case_line[4] = (input_gx + unit < input_gs);
                
                case_line[5] = (input_x >= unit);
                case_line[6] = (input_x + 5 * unit <= AnchorBlockSizeX * numAnchorBlockX);
                case_line[7] = (input_gx + 5 * unit < input_gs);
                case_line[8] = case_line[3];
                
                
                // 预加载 shared memory 数据到寄存器
                T1 tmp[5];
                tmp[0] = *((T1*)s_data + s_id[0]); 
                tmp[1] = *((T1*)s_data + s_id[1]); 
                tmp[2] = *((T1*)s_data + s_id[2]); 
                tmp[3] = *((T1*)s_data + s_id[3]); 
                tmp[4] = *((T1*)s_data + s_id[4]); 
                

                #pragma unroll
                for(int i = 0; i < 2; ++i){
                    pred[i] = tmp[i + 1];
                    if ( (case_line[0] && case_line[1 + i * 4] && case_line[2 + i * 4]) || (!case_line[0] && case_line[1 + i * 4] && case_line[2 + i * 4] && case_line[3 + i * 4])) {
                        pred[i] = cubic_interpolator(tmp[0 + i], tmp[1 + i], tmp[2 + i], tmp[3 + i]);
                    }
                    else if ((case_line[0] && case_line[1 + i * 4] && !case_line[2 + i * 4]) || ( !case_line[0] && case_line[1 + i * 4] && !(case_line[2 + i * 4] && case_line[3 + i * 4]) && case_line[4 + i * 4])) {
                        pred[i] = (-tmp[0 + i] + 6 * tmp[1 + i] + 3 * tmp[2 + i]) / 8;
                    }
                    else if ((case_line[0] && !case_line[1 + i * 4] && case_line[2 + i * 4]) || (!case_line[0] && !case_line[1 + i * 4] && case_line[2 + i * 4] && case_line[3 + i * 4] )){
                        pred[i] = (3 * tmp[1 + i] + 6 * tmp[2 + i] - tmp[3 + i]) / 8;   
                    }
                    else if ((case_line[0] && !case_line[1 + i * 4] && !case_line[2 + i * 4]) || (!case_line[0] && !case_line[1 + i * 4] && !(case_line[2 + i * 4] && case_line[3 + i * 4]) && case_line[4 + i * 4])) {
                        pred[i] = (tmp[1 + i] + tmp[2 + i]) / 2;
                    }
                }
                ...
                if CONSTEXPR (WORKFLOW == SPLINE3_COMPR) {
                    for(int i = 0; i < 2; ++i){
                        if(xyz_predicate<SPLINE_DIM,
                            AnchorBlockSizeX, AnchorBlockSizeY, AnchorBlockSizeZ,
                            numAnchorBlockX, numAnchorBlockY, numAnchorBlockZ, BORDER_INCLUSIVE>(x + i * 2 * unit * I_X, y + i * 2 * unit * I_Y, z  + i * 2 * unit * I_Z, data_size)){
                        auto          err = *((T1*)s_data + x_size * y_size * z + x_size * y + x + stride * i) - pred[i];
                        decltype(err) code;
                        ...
                        *((T2*)s_ectrl + x_size * y_size * z + x_size * y + x + stride * i) = code;  // TODO double check if unsigned type works
                    
                        *((T1*)s_data + x_size * y_size * z + x_size * y + x + stride * i)  = pred[i] + (code - radius) * ebx2;
                    }
                    }
                    ...
                    for(int i = 0; i < 2; ++i){
                        ...
                        auto code       = *((T2*)s_ectrl + x_size * y_size * z + x_size * y + x + stride * i);
                        *((T1*)s_data + x_size * y_size * z + x_size * y + x + stride * i) = pred[i] + (code - radius) * ebx2;
            ...
            if CONSTEXPR (FACE) {  //
                ...
                if (I_YZ){
                 ...
                 x_1 = z,BI_1 = BIZ, GD_1 = GDZ, gx_1 = global_z,gs_1 = data_size.z;
                 x_2 = y,BI_2 = BIY, GD_2 = GDY, gx_2 = global_y, gs_2 = data_size.y;
                 s_id_1[0][0] = x_size * y_size * id_z[0] + x_size * y + x;
                 s_id_1[0][1] = x_size * y_size * id_z[1] + x_size * y + x;
                 s_id_1[0][2] = x_size * y_size * id_z[2] + x_size * y + x;
                 s_id_1[0][3] = x_size * y_size * id_z[3] + x_size * y + x;
                 s_id_1[0][4] = x_size * y_size * id_z[4] + x_size * y + x;
                 
                 s_id_2[0][0] = x_size * y_size * z + x_size * id_y[0] + x;
                 s_id_2[0][1] = x_size * y_size * z + x_size * id_y[1] + x;
                 s_id_2[0][2] = x_size * y_size * z + x_size * id_y[2] + x;
                 s_id_2[0][3] = x_size * y_size * z + x_size * id_y[3] + x;
                 s_id_2[0][4] = x_size * y_size * z + x_size * id_y[4] + x;
                 
                 s_id_1[1][0] = x_size * y_size * id_z[0] + x_size * (y + 2 * unit) + x;
                 s_id_1[1][1] = x_size * y_size * id_z[1] + x_size * (y + 2 * unit) + x;
                 s_id_1[1][2] = x_size * y_size * id_z[2] + x_size * (y + 2 * unit) + x;
                 s_id_1[1][3] = x_size * y_size * id_z[3] + x_size * (y + 2 * unit) + x;
                 s_id_1[1][4] = x_size * y_size * id_z[4] + x_size * (y + 2 * unit) + x;
                 
                 s_id_2[1][0] = x_size * y_size * (z + 2 * unit) + x_size * id_y[0] + x;
                 s_id_2[1][1] = x_size * y_size * (z + 2 * unit) + x_size * id_y[1] + x;
                 s_id_2[1][2] = x_size * y_size * (z + 2 * unit) + x_size * id_y[2] + x;
                 s_id_2[1][3] = x_size * y_size * (z + 2 * unit) + x_size * id_y[3] + x;
                 s_id_2[1][4] = x_size * y_size * (z + 2 * unit) + x_size * id_y[4] + x;

                 pred[0] = s_data[id_z[1]][id_y[1]][x];
                 pred[1] = s_data[id_z[1]][id_y[2]][x];
                 pred[2] = s_data[id_z[2]][id_y[1]][x];
                 pred[3] = s_data[id_z[2]][id_y[2]][x];

                stride_1 = x_size * y_size * 2 * unit;
                stride_2 = x_size * 2 * unit;
                    ...
                    s_id_2[0][0] = x_size * y_size * z + x_size * y + id_x[0];
                    s_id_2[0][1] = x_size * y_size * z + x_size * y + id_x[1];
                    s_id_2[0][2] = x_size * y_size * z + x_size * y + id_x[2];
                    s_id_2[0][3] = x_size * y_size * z + x_size * y + id_x[3];
                    s_id_2[0][4] = x_size * y_size * z + x_size * y + id_x[4];
                    
                    s_id_1[1][0] = x_size * y_size * id_z[0] + x_size * y + (x + 2 * unit);
                    s_id_1[1][1] = x_size * y_size * id_z[1] + x_size * y + (x + 2 * unit);
                    s_id_1[1][2] = x_size * y_size * id_z[2] + x_size * y + (x + 2 * unit);
                    s_id_1[1][3] = x_size * y_size * id_z[3] + x_size * y + (x + 2 * unit);
                    s_id_1[1][4] = x_size * y_size * id_z[4] + x_size * y + (x + 2 * unit);
                    
                    s_id_2[1][0] = x_size * y_size * (z + 2 * unit) + x_size * y + id_x[0];
                    s_id_2[1][1] = x_size * y_size * (z + 2 * unit) + x_size * y + id_x[1];
                    s_id_2[1][2] = x_size * y_size * (z + 2 * unit) + x_size * y + id_x[2];
                    s_id_2[1][3] = x_size * y_size * (z + 2 * unit) + x_size * y + id_x[3];
                    s_id_2[1][4] = x_size * y_size * (z + 2 * unit) + x_size * y + id_x[4];
                    
                    pred[0] = s_data[id_z[1]][y][id_x[1]];
                    pred[1] = s_data[id_z[1]][y][id_x[2]];
                    pred[2] = s_data[id_z[2]][y][id_x[1]];
                    pred[3] = s_data[id_z[2]][y][id_x[2]];
                    ...
                    x_1 = y,BI_1 = BIY, GD_1 = GDY, gx_1 = global_y, gs_1 = data_size.y;
                    x_2 = x,BI_2 = BIX, GD_2 = GDX, gx_2 = global_x, gs_2 = data_size.x;
                    s_id_1[0][0] = x_size * y_size * z + x_size * id_y[0] + x;
                    s_id_1[0][1] = x_size * y_size * z + x_size * id_y[1] + x;
                    s_id_1[0][2] = x_size * y_size * z + x_size * id_y[2] + x;
                    s_id_1[0][3] = x_size * y_size * z + x_size * id_y[3] + x;
                    s_id_1[0][4] = x_size * y_size * z + x_size * id_y[4] + x;
                    ...
                    s_id_1[1][0] = x_size * y_size * z + x_size * id_y[0] + (x + 2 * unit);
                    s_id_1[1][1] = x_size * y_size * z + x_size * id_y[1] + (x + 2 * unit);
                    s_id_1[1][2] = x_size * y_size * z + x_size * id_y[2] + (x + 2 * unit);
                    s_id_1[1][3] = x_size * y_size * z + x_size * id_y[3] + (x + 2 * unit);
                    s_id_1[1][4] = x_size * y_size * z + x_size * id_y[4] + (x + 2 * unit);


                    s_id_2[1][0] = x_size * y_size * z + x_size * (y + 2 * unit) + id_x[0];
                    s_id_2[1][1] = x_size * y_size * z + x_size * (y + 2 * unit) + id_x[1];
                    s_id_2[1][2] = x_size * y_size * z + x_size * (y + 2 * unit) + id_x[2];
                    s_id_2[1][3] = x_size * y_size * z + x_size * (y + 2 * unit) + id_x[3];
                    s_id_2[1][4] = x_size * y_size * z + x_size * (y + 2 * unit) + id_x[4];
                    
                    pred[0] = s_data[z][id_y[1]][id_x[1]];
                    pred[1] = s_data[z][id_y[1]][id_x[1] + 2 * unit];
                    pred[2] = s_data[z][id_y[1] + 2 * unit][id_x[1]];
                    pred[3] = s_data[z][id_y[1] + 2 * unit][id_x[2] + 2 * unit];

                    stride_1 = x_size * 2 * unit;
                    stride_2 = 2 * unit;
                }
                    int interp_1[2];
                    int interp_2[2];

                    interp_1[0] = get_interp_order(x_1, BI_1, GD_1, gx_1, gs_1);
                    interp_2[0] = get_interp_order(x_2, BI_2, GD_2, gx_2, gs_2);

                    interp_1[1] = get_interp_order(x_1 + 2 * unit, BI_1, GD_1, gx_1 + 2 * unit, gs_1);
                    interp_2[1] = get_interp_order(x_2 + 2 * unit, BI_2, GD_2, gx_2 + 2 * unit, gs_2);
                    ...
                    #pragma unroll
                    for(int i = 0; i < 5; ++i){
                        tmp_1[0][i] = *((T1*)s_data + s_id_1[0][i]);
                        tmp_2[0][i] = *((T1*)s_data + s_id_2[0][i]);
                        tmp_1[1][i] = *((T1*)s_data + s_id_1[1][i]);
                        tmp_2[1][i] = *((T1*)s_data + s_id_2[1][i]);
                    }
                    ...
                    for (int i = 0; i < 2; ++i){
                        ...
                        for(int j = 0; j < 2; ++j){
                            ...
                            if (interp_1[i] == 4 && interp_2[j] == 4) {
                                pred[i * 2 + j] = (cubic_interpolator(tmp_1[j][i], 
                                tmp_1[j][i + 1], 
                                tmp_1[j][i + 2], 
                                tmp_1[j][i + 3]) +
                                cubic_interpolator(tmp_2[i][j], 
                                tmp_2[i][j + 1], 
                                tmp_2[i][j + 2], 
                                tmp_2[i][j + 3])) / 2;
                            } else if (interp_1[i] != 4 && interp_2[j] == 4) {
                                pred[i * 2 + j] = cubic_interpolator(tmp_2[i][j], 
                                tmp_2[i][j + 1], 
                                tmp_2[i][j + 2], 
                                tmp_2[i][j + 3]);
                            } else if (interp_1[i] == 4 && interp_2[j] != 4) {
                                ...
                                tmp_1[j][i + 2], 
                                tmp_1[j][i + 3]);
                            } else if (interp_1[i] == 3 && interp_2[j] == 3) {
                                pred[i * 2 + j] = (-(tmp_2[i][j])+6*(tmp_2[i][j + 1]) + 3*(tmp_2[i][j + 2])) / 8;
                                pred[i * 2 + j] += (-(tmp_1[j][i])+6*(tmp_1[j][i + 1]) + 3*(tmp_1[j][i + 2])) / 8;
                                pred[i * 2 + j] /= 2;
                            } else if (interp_1[i] == 3 && interp_2[j] == 2) {
                                pred[i * 2 + j] = (3*(tmp_2[i][j + 1])+6*(tmp_2[i][j + 2]) - (tmp_2[i][j + 3])) / 8;
                                pred[i * 2 + j] += (-(tmp_1[j][i])+6*(tmp_1[j][i + 1]) + 3*(tmp_1[j][i + 2])) / 8;
                                pred[i * 2 + j] /= 2;
                            } else if (interp_1[i] == 3 && interp_2[j] < 2) {
                                pred[i * 2 + j] = (-(tmp_1[j][i])+6*(tmp_1[j][i + 1]) + 3*(tmp_1[j][i + 2])) / 8;
                            } else if (interp_1[i] == 2 && interp_2[j] == 3) {
                                pred[i * 2 + j] = (3*(tmp_1[j][i + 1])+6*(tmp_1[j][i + 2]) - (tmp_1[j][i + 3])) / 8;
                                pred[i * 2 + j] += (-(tmp_2[i][j])+6*(tmp_2[i][j + 1]) + 3*(tmp_2[i][j + 2])) / 8;
                                pred[i * 2 + j] /= 2;
                            } else if (interp_1[i] == 2 && interp_2[j] == 2) {
                                pred[i * 2 + j] = (3*(tmp_1[j][i + 1])+6*(tmp_1[j][i + 2]) - (tmp_1[j][i + 3])) / 8;
                                pred[i * 2 + j] += (3*(tmp_2[i][j + 1])+6*(tmp_2[i][j + 2]) - (tmp_2[i][j + 3])) / 8;
                                pred[i * 2 + j] /= 2;
                            } else if (interp_1[i] == 2 && interp_2[j] < 2) {
                                pred[i * 2 + j] = (3*(tmp_1[j][i + 1])+6*(tmp_1[j][i + 2]) - (tmp_1[j][i + 3])) / 8;
                            } else if (interp_1[i] <= 1 && interp_2[j] == 3) {
                                pred[i * 2 + j] = (-(tmp_2[i][j])+6*(tmp_2[i][j + 1]) + 3*(tmp_2[i][j + 2])) / 8;
                            } else if (interp_1[i] <= 1 && interp_2[j] == 2) {
                                pred[i * 2 + j] = (3*(tmp_2[i][j + 1])+6*(tmp_2[i][j + 2]) - (tmp_2[i][j + 3])) / 8;
                            } else if (interp_1[i] == 1 && interp_2[j] == 1) {
                                pred[i * 2 + j] = ((tmp_2[i][j + 1])+(tmp_2[i][j + 2])) / 2;
                                pred[i * 2 + j] += ((tmp_1[j][i + 1])+(tmp_1[j][i + 2])) / 2;
                                pred[i * 2 + j] /= 2;
                            } else if (interp_1[i] == 1 && interp_2[j] < 1) {
                                
                                pred[i * 2 + j] = ((tmp_1[j][i + 1])+(tmp_1[j][i + 2])) / 2;
                            } else if (interp_1[i] == 0 && interp_2[j] == 1) {
                                pred[i * 2 + j] = ((tmp_2[i][j + 1])+(tmp_2[i][j + 2])) / 2;
                            }
                            else{
                                pred[i * 2 + j] = (tmp_1[j][i + 1]) + (tmp_2[i][j + 1]) - pred[i * 2 + j];
                            }

                    // auto interp_1 = get_interp_order(x_1 + 2 * unit * i, BI_1, GD_1, gx_1 + 2 * unit, gs_1);
                    // auto interp_2 = get_interp_order(x_2 + 2 * unit * j, BI_2, GD_2, gx_2 + 2 * unit, gs_2);
                    // auto pred_ = pred[i * 2 + j];

                    // if (interp_1 == 4 && interp_2 == 4) {
                    //     pred_ = (cubic_interpolator(*((T1*)s_data + s_id_1[j][i]), 
                    //     *((T1*)s_data + s_id_1[j][i + 1]), 
                    //     *((T1*)s_data + s_id_1[j][i + 2]), 
                    //     *((T1*)s_data + s_id_1[j][i + 3])) +
                    //      cubic_interpolator(*((T1*)s_data + s_id_2[i][j]), 
                    //     *((T1*)s_data + s_id_2[i][j + 1]), 
                    //     *((T1*)s_data + s_id_2[i][j + 2]), 
                    //     *((T1*)s_data + s_id_2[i][j + 3]))) / 2;
                    // } else if (interp_1 != 4 && interp_2 == 4) {
                    //     pred_ = cubic_interpolator(*((T1*)s_data + s_id_2[i][j]), 
                    //     *((T1*)s_data + s_id_2[i][j + 1]), 
                    //     *((T1*)s_data + s_id_2[i][j + 2]), 
                    //     *((T1*)s_data + s_id_2[i][j + 3]));
                    // } else if (interp_1 == 4 && interp_2 != 4) {
                    //     pred_ = cubic_interpolator(*((T1*)s_data + s_id_1[j][i]), 
                    //     *((T1*)s_data + s_id_1[j][i + 1]), 
                    //     *((T1*)s_data + s_id_1[j][i + 2]), 
                    //     *((T1*)s_data + s_id_1[j][i + 3]));
                    // } else if (interp_1 == 3 && interp_2 == 3) {
                    //     pred_ = (-(*((T1*)s_data + s_id_2[i][j]))+6*(*((T1*)s_data + s_id_2[i][j + 1])) + 3*(*((T1*)s_data + s_id_2[i][j + 2]))) / 8;
                    //     pred_ += (-(*((T1*)s_data + s_id_1[j][i]))+6*(*((T1*)s_data + s_id_1[j][i + 1])) + 3*(*((T1*)s_data + s_id_1[j][i + 2]))) / 8;
                    //     pred_ /= 2;
                    // } else if (interp_1 == 3 && interp_2 == 2) {
                    //     pred_ = (3*(*((T1*)s_data + s_id_2[i][j + 1]))+6*(*((T1*)s_data + s_id_2[i][j + 2])) - (*((T1*)s_data + s_id_2[i][j + 3]))) / 8;
                    //     pred_ += (-(*((T1*)s_data + s_id_1[j][i]))+6*(*((T1*)s_data + s_id_1[j][i + 1])) + 3*(*((T1*)s_data + s_id_1[j][i + 2]))) / 8;
                    //     pred_ /= 2;
                    // } else if (interp_1 == 3 && interp_2 < 2) {
                    //     pred_ = (-(*((T1*)s_data + s_id_1[j][i]))+6*(*((T1*)s_data + s_id_1[j][i + 1])) + 3*(*((T1*)s_data + s_id_1[j][i + 2]))) / 8;
                    // } else if (interp_1 == 2 && interp_2 == 3) {
                    //     pred_ = (3*(*((T1*)s_data + s_id_1[j][i + 1]))+6*(*((T1*)s_data + s_id_1[j][i + 2])) - (*((T1*)s_data + s_id_1[j][i + 3]))) / 8;
                    //     pred_ += (-(*((T1*)s_data + s_id_2[i][j]))+6*(*((T1*)s_data + s_id_2[i][j + 1])) + 3*(*((T1*)s_data + s_id_2[i][j + 2]))) / 8;
                    //     pred_ /= 2;
                    // } else if (interp_1 == 2 && interp_2 == 2) {
                    //     pred_ = (3*(*((T1*)s_data + s_id_1[j][i + 1]))+6*(*((T1*)s_data + s_id_1[j][i + 2])) - (*((T1*)s_data + s_id_1[j][i + 3]))) / 8;
                    ...
                    //     pred_ = (3*(*((T1*)s_data + s_id_2[i][j + 1]))+6*(*((T1*)s_data + s_id_2[i][j + 2])) - (*((T1*)s_data + s_id_2[i][j + 3]))) / 8;
                    // } else if (interp_1 == 1 && interp_2 == 1) {
                    //     pred_ = ((*((T1*)s_data + s_id_2[i][j + 1]))+(*((T1*)s_data + s_id_2[i][j + 2]))) / 2;
                    //     pred_ += ((*((T1*)s_data + s_id_1[j][i + 1]))+(*((T1*)s_data + s_id_1[j][i + 2]))) / 2;
                    //     pred_ /= 2;
                    // } else if (interp_1 == 1 && interp_2 < 1) {
                    ...
                    // }
                    // else{
                    //     pred_ = (*((T1*)s_data + s_id_1[j][i + 1])) + (*((T1*)s_data + s_id_2[i][j + 1])) - pred_;
                    // }
                    if CONSTEXPR (WORKFLOW == SPLINE3_COMPR) {
                        // for(int i = 0; i < 2; ++i){
                        //     for(int j = 0; j < 2; ++j){
                                if(xyz_predicate<SPLINE_DIM,
                                    AnchorBlockSizeX, AnchorBlockSizeY, AnchorBlockSizeZ,
                                    numAnchorBlockX, numAnchorBlockY, numAnchorBlockZ, BORDER_INCLUSIVE>(x + j * 2 * unit, y + i * 2 * unit, z,data_size)){
                                    
                                    auto          err = *((T1*)s_data + x_size * y_size * z + x_size * y + x + stride_1 * i + stride_2 * j) - pred[i * 2 + j];
                                    decltype(err) code;
                                    ...
                                    s_ectrl[z][y + i * 2 * unit][x + j * 2 * unit] = code; 
                                    
                                    s_data[z][y + i * 2 * unit][x + j * 2 * unit]  = pred[i * 2 + j] + (code - radius) * ebx2;
                                }
                        //     }
                        // }
                        ...
                        //     for(int j = 0; j < 2; ++j){
                                if(xyz_predicate<SPLINE_DIM,
                                    AnchorBlockSizeX, AnchorBlockSizeY, AnchorBlockSizeZ,
                                    numAnchorBlockX, numAnchorBlockY, numAnchorBlockZ, BORDER_INCLUSIVE>(x + j * 2 * unit, y + i * 2 * unit, z,data_size)){
                                    auto code       = *((T2*)s_ectrl + x_size * y_size * z + x_size * y + x + stride_1 * i + stride_2 * j);
                                    *((T1*)s_data + x_size * y_size * z + x_size * y + x + stride_1 * i + stride_2 * j) = pred[i * 2 + j] + (code - radius) * ebx2;
    ...
    if CONSTEXPR (COARSEN) {
        ...
        auto TOTAL = NUM_ELE;
        for (auto _tix = TIX; _tix * ele_per_thread < TOTAL; _tix += LINEAR_BLOCK_SIZE) {
            auto [x,y,z]    = xyzmap(_tix, unit);
            run(x, y, z);
        }
...
template<int SPLINE_DIM, int BLOCKSIZE>
__device__ std::tuple<int, int, int> xyzmap_line_1(int _tix, const int UNIT) {
    ...
    if constexpr (SPLINE_DIM == 2) {
        auto N = BLOCKSIZE / (UNIT * 2);
        auto M = (BLOCKSIZE + UNIT * 2 * 2 - 1) / (UNIT * 2 * 2);
        auto L = M * (N + 1); 
        auto Q = (N + 1); 
        auto group = _tix / L ;
        auto m = _tix % L ;
        ...
        auto j = (m % Q);
        if(group == 0)
            return std::make_tuple(2 * 2 * UNIT * i + UNIT, 2 * UNIT * j, 0);
        else if (group == 1)
            return std::make_tuple(2 * UNIT * j, 2 * 2 * UNIT * i + UNIT, 0);
    }
}

template<int SPLINE_DIM, int BLOCKSIZE>
__device__ std::tuple<int, int, int> xyzmap_face_1(int _tix, const int UNIT) {
    if constexpr (SPLINE_DIM == 3) {
        auto N = BLOCKSIZE / (UNIT * 2);
        auto L = N * N * (N+1);
        auto Q = N * N; 
    ...
    if constexpr (SPLINE_DIM == 2) {
        auto N = (BLOCKSIZE + UNIT * 2 * 2 - 1) / (UNIT * 2 * 2);
        auto L = N * N;
        auto Q = N * N; 
        auto group = _tix / L ;
        auto m = _tix % L ;
        ...
        auto i = (m % Q) / N;
        auto j = (m % Q) % N;
        return std::make_tuple(2 * 2 * UNIT * i + UNIT, 2 * 2 * UNIT * j + UNIT, 0);
    }
}
    ...
    for(int unit = max_unit; unit >= 2; unit /= 2, level_id--){
        ...
        // else{
            ...
            calc_eb(unit);
                ...
                if(intp_param.use_natural[level_id]==0){
                    if constexpr (SPLINE_DIM >= 1)
                    interpolate_stage_md_1<T1, T2, FP, SPLINE_DIM, AnchorBlockSizeX, AnchorBlockSizeY, AnchorBlockSizeZ, numAnchorBlockX, numAnchorBlockY, numAnchorBlockZ, decltype(xyzmap_line_1<SPLINE_DIM, AnchorBlockSizeX>), true, false, false, LINEAR_BLOCK_SIZE, COARSEN, BORDER_INCLUSIVE, WORKFLOW>(s_data, s_ectrl,data_size, xyzmap_line_1<SPLINE_DIM, AnchorBlockSizeX>, unit, cur_eb_r, cur_ebx2, radius, nan_cubic_interp, N_line);
                    if constexpr (SPLINE_DIM >= 2)
                    interpolate_stage_md_1<T1, T2, FP, SPLINE_DIM, AnchorBlockSizeX, AnchorBlockSizeY, AnchorBlockSizeZ, numAnchorBlockX, numAnchorBlockY, numAnchorBlockZ, decltype(xyzmap_face_1<SPLINE_DIM, AnchorBlockSizeX>), false, true, false, LINEAR_BLOCK_SIZE, COARSEN, BORDER_INCLUSIVE, WORKFLOW>(s_data, s_ectrl,data_size, xyzmap_face_1<SPLINE_DIM, AnchorBlockSizeX>, unit, cur_eb_r, cur_ebx2, radius, nan_cubic_interp, N_face);
                    if constexpr (SPLINE_DIM >= 3)
