    ...
    pszmem_cxx<T>* profiling_errors)
{
  ...
  // todo 2d: Auto Tuning kernel
  // Not know tuning logic, implement interpolation first
  ...
  //       CHECK_GPU(cudaMemcpy(profiling_errors->m->h, profiling_errors->m->d,
  //       profiling_errors->m->bytes, cudaMemcpyDeviceToHost)); auto
  //       errors=profiling_errors->hptr();
  ...
  //       bool
  //       do_reverse=(errors[4+intp_param.interpolators[2]]>3*errors[intp_param.interpolators[0]]);
  //        // bool do_reverse=(errors[1]>2*errors[0]);
  //        intp_param.reverse[0]=intp_param.reverse[1]=intp_param.reverse[2]=do_reverse;
  //     }
  //   }

  cusz::c_spline2d_infprecis_data<T*, E*, float, 8, 8, 1, 1, 1, 1, DEFAULT_BLOCK_SIZE>
    ...
    void* stream)
{
  ...
  cusz::x_spline2d_infprecis_data<
      E*, T*, float, 8, 8, 1, 1, 1, 1, DEFAULT_BLOCK_SIZE>                   //
      <<<grid_dim, dim3(DEFAULT_BLOCK_SIZE, 1, 1), 0, (GpuStreamT)stream>>>  //
      (ectrl->dptr(), ectrl->template len3<dim3>(),
       ectrl->template st3<dim3>(),  //
